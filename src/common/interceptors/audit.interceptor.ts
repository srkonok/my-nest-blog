...
export class AuditInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    this.logger.debug(`Processing request: ${method} ${url}`);
    if (user) {
      this.logger.debug(`User info: ${JSON.stringify(user)}`);
    } else {
      this.logger.debug('No user information available in request');
      if (isLoginRoute) {
        this.logger.debug(`Login attempt for email: ${userEmail}`);
      } else if (isRegisterRoute) {
        this.logger.debug(`Registration attempt for email: ${userEmail}`);
      }
    }

    // Get action and resource from decorators or infer from HTTP method
    let action = this.reflector.getAllAndOverride<AuditActionType>(AUDIT_ACTION_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    let resource = this.reflector.getAllAndOverride<string>(AUDIT_RESOURCE_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    this.logger.debug(`Audit data prepared: ${JSON.stringify({
      ...
      tap({
        ...
        error: (error) => {
          ...
          this.auditService.log(auditData).catch(logError => {
            this.logger.error(`Failed to log audit: ${logError.message}`, logError.stack);
          });
        },
      }),
    );
